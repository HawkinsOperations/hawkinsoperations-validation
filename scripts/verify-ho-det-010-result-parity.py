#!/usr/bin/env python3
"""Verify stable HO-DET-010 validation-result parity."""
...
VALIDATOR_PATH = ROOT / "scripts" / "validate-ho-det-010.py"
VALIDATION_CASES = ROOT / "validation" / "successor" / "ho-det-010" / "validation-cases.json"
VALIDATION_RESULT = ROOT / "reports" / "ho-det-010" / "validation-result.json"
def fail(message: str) -> None:
    print(f"FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
def load_validator():
    spec = importlib.util.spec_from_file_location("validate_ho_det_010", VALIDATOR_PATH)
    if spec is None or spec.loader is None:
        fail("could not load HO-DET-010 validator")
...
def main() -> int:
    ...
    if actual != expected:
        fail("HO-DET-010 validation result is out of date")
    print("STATUS=pass")
