#!/usr/bin/env python3
"""Verify stable ID-DET-003 validation-result parity."""
...
VALIDATOR_PATH = ROOT / "scripts" / "validate-id-det-003.py"
VALIDATION_CASES = ROOT / "validation" / "identity" / "id-det-003" / "validation-cases.json"
VALIDATION_RESULT = ROOT / "reports" / "id-det-003" / "validation-result.json"
...
def load_validator():
    spec = importlib.util.spec_from_file_location("validate_id_det_003", VALIDATOR_PATH)
    if spec is None or spec.loader is None:
        fail("could not load ID-DET-003 validator module")
