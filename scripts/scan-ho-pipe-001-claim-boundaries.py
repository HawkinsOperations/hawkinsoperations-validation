#!/usr/bin/env python3
"""Scan scoped HO-PIPE-001 validation artifacts for claim-boundary violations."""
...
SCOPED_FILES = [
    ROOT / "validation" / "successor" / "ho-pipe-001" / "README.md",
    ROOT / "validation" / "successor" / "ho-pipe-001" / "validation-cases.json",
    ROOT / "reports" / "ho-pipe-001" / "validation-result.json",
    ROOT / "reports" / "ho-pipe-001" / "validation-result.md",
]
BLOCKED_TERMS = [
    "runtime-active",
    "signal-observed",
...
def iter_json_strings(value: Any, path: str = "") -> list[tuple[str, str]]:
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            found.extend(iter_json_strings(item, child))
