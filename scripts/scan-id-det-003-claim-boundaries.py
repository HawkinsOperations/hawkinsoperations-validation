#!/usr/bin/env python3
"""Scan scoped ID-DET-003 validation artifacts for claim-boundary violations."""
...
SCOPED_FILES = [
    ROOT / "validation" / "identity" / "id-det-003" / "README.md",
    ROOT / "validation" / "identity" / "id-det-003" / "validation-cases.json",
    ROOT / "reports" / "id-det-003" / "validation-result.json",
    ROOT / "reports" / "id-det-003" / "validation-result.md",
]
BLOCKED_TERMS = [
    "runtime-active",
    "signal-observed",
...
ALLOWED_JSON_PATH_PARTS = {"blocked_claims", "trust_boundary", "privacy_status", "public_safe_status", "not_claimed_here"}
ALLOWED_TEXT_MARKERS = [
    ...
    "do not prove",
    "not_public_safe",
    "controlled identity administration fixtures only",
...
def context_allowed(path: str, text: str) -> bool:
    ...
    return (
        any(part in lower_path for part in ALLOWED_JSON_PATH_PARTS)
        or lower_text in {"blocked", "no", "not_public_safe", "not_proven", "false"}
        or any(marker in lower_text for marker in ALLOWED_TEXT_MARKERS)
    )
...
def scan_json_file(path: Path, text: str) -> None:
    ...
    for field_path, value in iter_json_strings(data):
        ...
        for term in BLOCKED_TERMS:
            if term.lower() in lower and not context_allowed(field_path, value):
...
def markdown_section(line: str, current: str) -> str:
    ...
    return match.group(2).strip().lower() if match else current


def markdown_context_allowed(section: str, line: str) -> bool:
    lower = line.lower()
    if section in {"blocked claims", "boundary", "validation boundary", "not claimed here"}:
        return True
