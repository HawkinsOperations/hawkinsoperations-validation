#!/usr/bin/env python3
"""Controlled-test validation runner for HO-DET-013."""
...
SOURCE_DIR = DETECTIONS_ROOT / "detections" / "successor" / "ho-det-013"
CASES_FILE = ROOT / "validation" / "successor" / "ho-det-013" / "validation-cases.json"
REPORT_DIR = ROOT / "reports" / "ho-det-013"
REPORT_JSON = REPORT_DIR / "validation-result.json"
REPORT_MD = REPORT_DIR / "validation-result.md"
SUPPORTED_CLAIM = "HO-DET-013 passed controlled-test validation against controlled telemetry/security-control tamper fixtures."
...
SECURITY_TARGETS = ("wazuhsvc", "sysmon", "sysmon64", "windefend", "eventlog", "sense", "securityhealthservice")
TAMPER_COMMANDS = (
    "sc.exe stop",
    "sc stop",
    "net stop",
...
def command_text(event: dict[str, Any]) -> str:
    values = [str(event.get(key, "") or "") for key in ("CommandLine", "Image", "ServiceName", "TargetService", "StartType", "Action")]
...
def event_matches(event: dict[str, Any]) -> bool:
    ...
    if eid == 1102:
        return True
    if eid in {7036, 7040}:
        approved = "approved" in lower(event, "ChangeWindow")
        target = any(item in lower(event, "ServiceName") for item in SECURITY_TARGETS)
        state = any(item in text for item in ("stopped", "disabled"))
        return target and state and not approved
    if eid == 1:
        if "approved" in lower(event, "ChangeWindow"):
            return False
        return any(item in text for item in TAMPER_COMMANDS) and (
            any(item in text for item in SECURITY_TARGETS)
            or "disable" in text
            or "clear" in text
            or "auditpol" in text
            or "set-mppreference" in text
        )
    return False


def validate_source_contract(mode: str) -> None:
    required = [SOURCE_DIR / "rule.yml", SOURCE_DIR / "event-mapping.yml", SOURCE_DIR / "status.yml"]
    ...
    for fragment in ("detection_id: HO-DET-013", "selection_log_clear_command", "selection_defender_tamper_command", "selection_service_event_target"):
...
def validate_fixture_contract(cases: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if cases.get("detection_id") != "HO-DET-013":
        fail("validation cases detection_id must be HO-DET-013")
    ...
    if len(positives) != EXPECTED_POSITIVE_COUNT or len(negatives) != EXPECTED_NEGATIVE_COUNT:
        fail("unexpected HO-DET-013 fixture counts")
...
def build_report(cases: dict[str, Any], source_contract: str = "required") -> dict[str, Any]:
    ...
    return {
        **controlled_report_contract(
        ...
        "source_reference": "hawkinsoperations-detections/detections/successor/ho-det-013",
        "validation_cases_file": "hawkinsoperations-validation/validation/successor/ho-det-013/validation-cases.json",
        ...
        "trust_boundary": "Controlled-test telemetry/security-control tamper fixture validation only. This does not prove runtime, signal, public-safe proof, live SIEM ingestion, production readiness, fleet-wide deployment, autonomous SOC behavior, AI-approved disposition, or analyst-approved disposition.",
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# HO-DET-013 Controlled-test Validation Result",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate HO-DET-013 controlled tamper fixtures.")
    parser.add_argument("--write", action="store_true")
    parser.add_argument("--source-contract", choices=("required", "skip-if-missing"), default="required")
    args = parser.parse_args()
    cases = load_json(CASES_FILE, "HO-DET-013 validation cases")
    report = build_report(cases, source_contract=args.source_contract)
    if args.write:
        write_reports(report)
    elif not REPORT_JSON.exists() or load_json(REPORT_JSON, "HO-DET-013 validation result") != report:
        fail("reports/ho-det-013/validation-result.json is out of date; run with --write")
